...
def adminSys():
    ...
    @app.get("/admin/edit")
    async def showFileCanEdit(adminToken=Cookie(""), soundTableIndex:int=1):
        """
        หน้าแก้ไขไฟล์
        + Input
            + adminToken
        ...
        if config["with_gform_and_gsheet"]["on"]:
            ...
            try:
                ...
                data_sound_table = ""
                if len(data_sound)==0:
                    ct = render_templates(ct, {"soundData": f"Don't Have Data In {config['local_storage']['sound data']}"})
                else:
                    if len(data_sound) < (soundTableIndex-1)*5 or soundTableIndex == 0:
                        data_sound_table += "<tr><td>Not Content Here</td></tr>"
                    key = list(data_sound.keys())[(soundTableIndex-1)*5:soundTableIndex*5]
                    for sound_id in key:
                    ...
                    data_sound_table = render_templates(
                        ...
                        data={
                            "soundDataRows": data_sound_table,
                            "soundTableIndex": soundTableIndex,
                            "lenData": math.ceil(len(data_sound)/5) if len(data_sound) != 0 else 1
                            })
